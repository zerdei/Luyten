...
public class TextOutputFormatter implements IOutputFormatter {
	...
	/**
	 * whether or not to emit debug line number comments
	...
	/**
	 * when writing out line numbers, keeps track of the
	 * most recently used one to avoid redundancy
	 */
	...
	/**
	 * maps original line numbers to decompiler-emitted line
	 * numbers and columns
	 */
	...
	public void startNode(final AstNode node) {
		...
		if (offset != Expression.MYSTERY_OFFSET) {
			...
			if (output instanceof PlainTextOutputWithBytecodeToLineNumberTable) {
				((PlainTextOutputWithBytecodeToLineNumberTable) output)
						.registerGeneratedLineNumber(node, offset, output.getRow());
			}
		}
	...
	/**
	 * Returns the mapping from original to
	 * decompiler-emitted line numbers.
	 */
